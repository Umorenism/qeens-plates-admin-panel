...
export default function OrdersPage() {
  ...
  // Sync data on Search/Filter change
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      fetchOrders(activeFilter, searchTerm, true);
    }, 300);
  ...
  // REFRESH LOGIC: Re-fetch when the window gains focus (coming back from Detail Page)
  useEffect(() => {
    const onFocus = () => fetchOrders(activeFilter, searchTerm, false);
