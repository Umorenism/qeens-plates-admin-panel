...
export default function Settings() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      setFetching(true);
      // NOTE: If the API uses GET to fetch, change .post to .get
      const res = await axios.post(`${API_BASE}/superadmin/setting`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        {
          name: formData.name,
          delivery_fee: Number(formData.delivery_fee), // Ensure number
          tax_rate: Number(formData.tax_rate),       // Ensure number
          delivery_time: String(formData.delivery_time), // API expects string "300"
          email: formData.email,
          phone: formData.phone,
        },
        { headers: { Authorization: `Bearer ${token}` } }
