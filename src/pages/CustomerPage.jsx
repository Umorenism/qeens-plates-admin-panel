...
// //             total: o.total,
// //             date: o.date,
// //             status: apiToUiStatus[o.status] || o.status // Apply Mapping
// //         }));
...
// //           stats: {
// //             total_orders: customerOrders.length,
// //             pending_orders: customerOrders.filter(o => o.status !== "Delivered").length,
// //             completion_rate: "100%"
// //           },
// //           recent_orders: customerOrders,
// //           recent_reviews: customerData?.recent_reviews || [] 
...
// //                           <span className={`px-3 py-1 rounded-full text-[10px] font-bold border uppercase tracking-tight ${statusStyles[order.status] || "bg-gray-100 text-gray-600"}`}>
// //                             {order.status}
// //                           </span>
// //                         </td>
// //                         <td className="px-4 py-4 text-gray-800 font-bold">₦{order.total}</td>
...
// /* ... (Keep original imports and helper components/styles) ... */
...
//           if (!uniqueMap.has(customerName)) {
//             uniqueMap.set(customerName, {
//               id: o.customer?.id || customerName, // Use actual ID if available
//               actualOrderId: o.id,
//               name: customerName,
//               order_id: o.order_id,
//               status: isActive ? "Active" : "Inactive" 
...
//       const apiData = customerRes?.data;

//       if (apiData) {
//         setCustomerDetails({
//           profile: {
//             name: apiData.profile.name,
//             location: apiData.profile.location,
//             email: apiData.profile.email,
//             phone: apiData.profile.phone,
//             customer_since: apiData.profile.customer_since,
//             avatar: apiData.profile.avatar
//           },
//           stats: {
//             total_orders: apiData.stats.total_orders,
//             pending_orders: apiData.stats.pending_orders,
//             completion_rate: apiData.stats.completion_rate
//           },
//           recent_orders: apiData.recent_orders.map(o => ({
//             order_id: o.order_id,
//             item: o.item,
//             total: o.total,
//             date: o.date,
//             status: "Order Received" // Defaulting since API sample didn't show per-order status
//           })),
//           recent_reviews: apiData.recent_reviews || [] 
//         });
//       }
//     } catch (err) {
//       console.error("Details Fetch Error:", err);
...
//   /* ... (Keep filteredCustomers useMemo logic) ... */

//   return (
//     <div className="flex min-h-screen bg-[#F6F1E8]">
//       <ReviewsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} reviews={recentReviews} customerName={profile.name} />

//       {/* SIDEBAR - remains unchanged except mapping logic above */}
//       <div className="w-[300px] mt-14 bg-[#F9F9F9] p-5 border-r flex flex-col shrink-0">
//         <h1 className="text-lg font-semibold text-gray-800 mb-4 font-roboto">Customers</h1>
//         <div className="relative mb-5">
//           <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
...
//                 <div className="min-w-0">
//                   <h2 className="font-bold text-2xl text-gray-800 truncate">{profile.name}</h2>
//                   <p className="text-sm text-gray-500 mb-1">{profile.location === "N/A" ? "No location set" : profile.location} • Joined {profile.customer_since}</p>
//                   <p className="text-xs text-gray-400 truncate">{profile.email} • {profile.phone}</p>
//                 </div>
//               </div>
...
//                       <tr key={i} className="hover:bg-gray-50/50 transition-colors">
//                         <td className="px-4 py-4 font-mono text-xs text-gray-600">{order.order_id}</td>
//                         <td className="px-4 py-4 capitalize font-medium text-gray-700">{order.item}</td>
//                         <td className="px-4 py-4 text-gray-800 font-bold">₦{order.total}</td>
//                         <td className="px-4 py-4 text-gray-500 text-xs">{order.date}</td>
//                       </tr>
//                     ))}
...
import { FaStar } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import { getAllOrders, getCustomerById } from "../api/apiServices"; 
...
export default function CustomerPage() {
  ...
  // Sync sidebar with customer IDs from orders
  const fetchSidebarFromOrders = async () => {
    setSidebarLoading(true);
    try {
      const response = await getAllOrders();
      ...
      if (ordersList.length > 0) {
        ...
        ordersList.forEach((o) => {
          const customerName = o.customer?.name || "Unknown Customer";
          const customerId = o.customer?.id; // Assuming API provides ID here
          const isActive = o.status !== "delivered" && o.status !== "Cancelled";
          
          if (!uniqueMap.has(customerName)) {
            uniqueMap.set(customerName, {
              id: customerId || 3, // Defaulting to 3 based on your request if ID is missing
              name: customerName,
              order_id: o.order_id,
              status: isActive ? "Active" : "Inactive" 
            });
  ...
  // Fetch full details from /api/v1/admin/customers/{id}
  const fetchCustomerDetails = async (id) => {
    setDetailsLoading(true);
    try {
      const response = await getCustomerById(id);
  ...
  /* ===========================
      Safe Data Access (Fixes ReferenceError)
     =========================== */
  const profile = customerDetails?.profile || { 
    name: "Select a customer", 
    location: "N/A", 
