...
//       // Adjust 'response.data.role' based on your exact API response structure
...
//         <div className="absolute right-24 top-1/2 -translate-y-1/2 z-20 w-full max-w-[520px]">
//           <div className="p-10">
//             <h2 className="text-[48px] font-[900] font-dm text-[#3A3A3A] text-center mb-10 font-dm">
//               Admin Login
//             </h2>
...
//                 disabled={loading}
//                 onChange={(e) => setEmail(e.target.value)}
//                 className="w-full px-6 py-4 rounded-[40px] rounded-tl-[5px] bg-gray-100 border border-gray-200 focus:border-[#a61c2f] outline-none transition-all text-gray-700 disabled:opacity-50 font-dm"
//               />
...
//                   disabled={loading}
//                   onChange={(e) => setPassword(e.target.value)}
//                   className="w-full rounded-tl-[5px] px-6 py-4 rounded-[40px] bg-gray-100 border border-gray-200 focus:border-[#a61c2f] outline-none transition-all text-gray-700 pr-12 disabled:opacity-50 font-dm"
//                 />
//                 <span
//                   onClick={() => setShowPassword(!showPassword)}
//                   className="absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
...
//                 onClick={handleLogin}
//                 disabled={loading}
//                 className="w-full py-2 bg-[#a61c2f] hover:bg-[#8d1828] text-white text-lg font-medium rounded-[12px] shadow-lg transition-all duration-200 mt-6 disabled:bg-gray-400 font-dm"
//               >
//                 {loading ? "Logging in..." : "Login"}
//               </button>

//               <Link to="/forget-password" intrinsic="forgot" className="w-full flex justify-end">
//                 <p className="text-sm text-red-500 hover:text-gray-700 transition-colors cursor-pointer font-dm">
//                   Forgot password?
//                 </p>
//               </Link>
//             </div>
...
export default function Login() {
  ...
  // 2. Destructure the login function from context
  const { login } = useAuth(); 
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 3. Extract data correctly from your apiServices response
      const token = response.token;
      const role = response.role;
      const userData = response.user;

      if (token) {
        // 4. CALL THE CONTEXT LOGIN
        // This updates the 'user' state globally so fetching starts immediately
      ...
      // Use the exact error message from your apiServices throw
  ...
  return (
    <div className="min-h-screen bg-[#e8d5b7] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative w-full h-screen bg-[#e8d5b7] overflow-hidden mx-auto font-dm">
        <img src={wave} alt="wave" className="absolute bottom-0 left-0 w-full h-[440px] object-cover z-[1]" />
        ...
        <div className="absolute right-24 top-1/2 -translate-y-1/2 z-20 w-full max-w-[520px]">
          <div className="p-10">
            <h2 className="text-[48px] font-[900] text-[#3A3A3A] text-center mb-10">
              Admin Login
            </h2>
